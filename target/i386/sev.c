...
static int sev_kvm_type(X86ConfidentialGuest *cg)
{
    ...
    if (kvm_is_vm_type_supported(kvm_type) &&
        (sev_snp_enabled() || !SEV_GUEST(sev_common)->legacy_vm_type)) {
...
#define SNP_REQ_CERTS_ERROR_INVALID_LEN   1
#define SNP_REQ_CERTS_ERROR_BUSY          2
#define SNP_REQ_CERTS_ERROR_GENERIC       (1 << 31)

static int kvm_handle_vmgexit_req_certs(__u64 gpa, __u64 *npages, __u32 *vmm_ret, __u8 *flags)
{
    ...
    *vmm_ret = SNP_REQ_CERTS_ERROR_GENERIC;
    ...
    if (buf_sz < sz) {
        *vmm_ret = SNP_REQ_CERTS_ERROR_INVALID_LEN;
...
int kvm_handle_vmgexit(struct kvm_run *run)
{
    ...
    if (run->vmgexit.type == KVM_USER_VMGEXIT_REQ_CERTS) {
        g_warning("%s: gpa %llx pages %lld flags %d status %d\n", __func__,
                  run->vmgexit.req_certs.data_gpa,
                  run->vmgexit.req_certs.data_npages,
                  run->vmgexit.req_certs.flags,
                  run->vmgexit.req_certs.status);

        ret = kvm_handle_vmgexit_req_certs(run->vmgexit.req_certs.data_gpa,
                                           &run->vmgexit.req_certs.data_npages,
